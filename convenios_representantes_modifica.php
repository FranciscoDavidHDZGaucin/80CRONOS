<?php
require_once('header_modifica.php');
...
$clientenombre2 = $fetchcliente2['CardName'];
     ...
     $fechafin = $_POST['fechafin'];
      ...
      $insertencabeza = sprintf("UPDATE encabeza_convenio SET fecha_alta=CURDATE(),"
              . "cve_cte=%s, nom_cte=%s,estatus=%s, n_agente=%s, observacion=%s, moneda=%s, plazo=%s, tipo_venta=%s,"
              . "total=%s,nom_age=%s,fact_ds=%s, fechainicio=%s, fechafin=%s,"
              . " por_regreso=%s, au_dc=0, au_an=0, au_ge=0, estatus='A' WHERE n_remision=%s ",
              GetSQLValueString($clienteconvenio2, "text"),
...
$tablaservicio3=mysqli_query($conecta1, $stringtabla) or die (mysqli_error($conecta1));
 ...
 $stringrecogerdatos = sprintf("SELECT * FROM encabeza_convenio WHERE n_remision = %s",
GetSQLValueString($_SESSION['foliorep'], "text"));
$queryrecogerdatos=mysqli_query($conecta1, $stringrecogerdatos) or die (mysqli_error($conecta1));
$fetchrecogerdatos = mysqli_fetch_assoc($queryrecogerdatos);


$comentariorecogido = $fetchrecogerdatos['observacion'];
$porcentajerecogido = $fetchrecogerdatos['por_regreso'];
$comentariorechazo = $fetchrecogerdatos['comentario_rechazo'];
...
<div class="container">
     <form name="forma1" method="POST" action="convenios_representantes_modifica.php?folio=<?php echo$_SESSION['foliorep'];?>">
    <div class="row">
        ...
        <h3>Folio: <?php echo $_SESSION['foliorep']; ?></h3>
         ...
         <h4>Comentario de rechazo: <?php echo $comentariorechazo; ?></h4>
       ...
       <div class="col-md-12">
           ...
           <label>Inicio</label>
           <input type="date" name="fechainicio" value="<?php echo $_SESSION['fechainicio'];?>">
           
           <label>Fin</label>
           <input type="date" name="fechafin" value="<?php echo $_SESSION['fechafin'];?>">
             <p>Cliente: </p>
...
<!--                     <th>Precio</th>-->
             ...
             <tbody>
                 ...
                 <tr>
                     ...
                     <td><a href="convenios_representantes_modifica.php?eliminar=<?php echo $registro1['id_detalle']; ?>&folio=<?php echo $_SESSION['foliorep']; ?>&fechainicio=<?php echo $_SESSION['fechainicio']; ?>&fechafin=<?php echo $_SESSION['fechafin']; ?>&cliente=<?php echo $_SESSION['cliente']; ?>"  onclick="return confirm('¿Está Seguro de Eliminar?')"><img src="images/eliminar.png"/></a></td>
...
<input type="hidden" name="totalservdist" value="<?php echo $totalsd ?>" class="form-control"  readonly >
    ...
    <div class=" col-lg-8" >
        <label for="comment">% de regreso si se cumple en tiempo:</label>

                <select name="por_regreso" >
                    <option  value="0">Elija</option>
                    <option <?php if($porcentajerecogido==1){ echo "selected";}  ?>  value="1">1%</option>
                    <option <?php if($porcentajerecogido==2){ echo "selected";}  ?>  value="2">2%</option>
                    <option <?php if($porcentajerecogido==3){ echo "selected";}  ?>  value="3">3%</option>
                    <option <?php if($porcentajerecogido==4){ echo "selected";}  ?>  value="4">4%</option>
                    <option <?php if($porcentajerecogido==5){ echo "selected";}  ?>  value="5">5%</option>
                    <option <?php if($porcentajerecogido==6){ echo "selected";}  ?>  value="6">6%</option>
                    <option <?php if($porcentajerecogido==7){ echo "selected";}  ?>  value="7">7%</option>
                    <option <?php if($porcentajerecogido==8){ echo "selected";}  ?>  value="8">8%</option>
                    <option <?php if($porcentajerecogido==9){ echo "selected";}  ?>  value="9">9%</option>
                    <option <?php if($porcentajerecogido==10){ echo "selected";}  ?> value="10">10%</option>
                </select>
        
        <p>
            <label for="comment">Comentario:</label>
  <textarea name="observacion" class="form-control" rows="5" id="comment"> <?php echo $comentariorecogido;?></textarea>
        </p>
    </div>
    <div class=" col-lg-12" >
        <p>
            <button name="guardar" type="submit" class="btn btn-lg btn-primary pull-right" onClick="alert('Convenio Guardado')">Guardar Convenio</button>
