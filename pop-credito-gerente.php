...
/*
********	 INFORMACION ARCHIVO ***************** 
	Nombre  Archivo : por-credito-gerente.php
 	Fecha  Creacion : ???
	Descripcion  : Escript  Encargodo  de la  Autorizacion de  Pedidos  
 * 
 *      Modificacion :
 *                    22/07/2017  Agregamos los Campos correspondientes al  Gestor dentro de  las Dos Opciones de AAutorizacion 
   ...
   if (isset($_REQUEST['actualizar'])){
       ...
       switch ($respuesta) {
           case 1:   ///Aqui se autoriza el pedido
                       ...
                       $fecha_comp= $_REQUEST['fecha_comp'];    //Folio del registro
                       $monto_comp= $_REQUEST['monto_comp'];    //Folio del registro
               
                         $update=  sprintf("UPDATE encabeza_pedido SET estatus='E', vbo_gerente=%s, comentario_gerente=%s, timeres_gerente=%s,fechacomp_gerente=%s, montocomp_gerente=%s ,vbo_gestor=%s, comentario_gestor=%s,timeres_gestor=%s WHERE id=%s",
                                     GetSQLValueString($respuesta, "int"),
                                     GetSQLValueString($comentario, "text"),
                                     GetSQLValueString($fechahora_hoy, "date"),  
                                     GetSQLValueString($fecha_comp, "date"),  
                                     GetSQLValueString($monto_comp, "double"), 
                                 /////****Campos  del  Gestor  
                                         GetSQLValueString($respuesta, "int"),
			   ...
			   $result0_datos=  mysqli_fetch_assoc($result0);     
				...
				$result2=mysqli_query($conecta1,$insertSQL) or die (mysqli_error($conecta1));
					...
					break;
						...
						$destinatario=email($usuario);
						$usuario="facforanea2";	
						$destinatario2[0]=email($usuario);
                                                 $usuario2="credito_aux";	
						$destinatario2[1]=$usuario2;
					...
					break; 
						...
						$subject="Remision Autorizada por Gerencia Ventas No: ".$result0_datos['n_remision'];
           ...
           case 2:   //Se pasa al siguiente usuario para que si es posible se autorice
                    //Actualizar la tabla encabezado pedido
                           $update=  sprintf("UPDATE encabeza_pedido SET vbo_gerente=%s, comentario_gerente=%s, timeres_gerente=%s,vbo_gestor=%s, comentario_gestor=%s,timeres_gestor=%s  WHERE id=%s",
                                     GetSQLValueString($respuesta, "int"),
                                     GetSQLValueString($comentario, "text"),
                                     GetSQLValueString($fechahora_hoy, "date"),
                                      /////****Campos  del  Gestor  
...
<script language="javascript" type="text/javascript">	  
 ...
 <body onunload="window.opener.location = window.opener.location;">
   ...
   <div class="container">   
             ...
             <form name="form1" id="form1" Method="POST" action="pop-credito-gerente.php" onsubmit="validar()">

                 <?php if ($respuesta==1){?>
                 <label>Fecha Compromiso</label>

                 <input type='date' name='fecha_comp' placeholder='YYYY-MM-DD' required><br>
                     <label>Monto Compromiso</label>         
                     <input type='number' name='monto_comp' placeholder='Monto Compromiso' required>
