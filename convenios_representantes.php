...
$_SESSION['foliorep'] = folio_convenio($_SESSION["usuario_agente"]);
...
$clientenombre = $fetchcliente['CardName'];
    ...
    $boni_costomp2=$fetchmp2['costo'];
      ...
      $insertproducto = sprintf("INSERT INTO detalle_convenio SET n_remision=%s, n_agente=%s, nom_age=%s, fecha_alta=CURDATE(), "
              ...
              . "boni_estado=%s, boni_costomp=%s, boni_bonificadomp=%s",
...
$clientenombre2 = $fetchcliente2['CardName'];
      ...
      $por_regreso = $_POST['por_regreso'];
      
      $observacion = $_POST['observacion'];
      
      $status='A';
 ////***Varible agregada para el 21/03/2017
       $tipo_conv = $_POST['tipo_conV'];      
      
      $insertencabeza = sprintf("INSERT INTO encabeza_convenio SET n_remision = %s, fecha_alta=CURDATE(),"
              . "cve_cte=%s, nom_cte=%s,estatus=%s, n_agente=%s, observacion=%s, moneda=%s, plazo=%s, tipo_venta=%s,"
              . "total=%s,nom_age=%s,fact_ds=%s, fechainicio=%s, fechafin=%s, por_regreso=%s ,tipo_convenio=%s ",
              GetSQLValueString($_SESSION['foliorep'], "int"),
              GetSQLValueString($clienteconvenio2, "text"),
              ...
              GetSQLValueString($por_regreso, "int"),
              GetSQLValueString($tipo_conv, "int"));
      
      $queryinsertencabeza=mysqli_query($conecta1, $insertencabeza) or die (mysqli_error($conecta1));
      
      ////*******Inicio  CodigoAgregado  para  Generar   Notificacion 
           //***Obtenemos el  correo  del  Agente  
            ////*****Cadena de  Consulta
            $string_mail_gerentes = sprintf("SELECT  zona,mail   FROM pedidos.relacion_gerentes where cve_age = %s",
             GetSQLValueString($idagente, "int")); 
            ///****** Hacemos la  consulta
            $qery_mail = mysqli_query($conecta1, $string_mail_gerentes);
            ...
            /////******Generamos  Cadenas para  el  Correo***************************
             ////*****Informacion a Enviar al  Gerente
                $STRING_CADENA_Html_001 = "<p><h4>Buen Día  se ha generado un Nuevo Convenio.</h4></p>";        
                $STRING_CADENA_Html_002 = "<p>Convenio  Folio: ".$_SESSION['foliorep']."</p>";
                $STRING_CADENA_Html_003 = "<p>N# Cliente: ".$clienteconvenio2."</p>";
                $STRING_CADENA_Html_004 = "<p>Cliente : ".$clientenombre2."</p>";
                $STRING_CADENA_Html_005 = "<p>Fecha Inicial: ".$fechainicio."  Fecha Final:".$fechafin."</p>"; 
                $STRING_CADENA_Html_006=  "<p>Monto de Convenio :". "$".number_format($totalprod2)."</p>";
                $STRING_CADENA_Html_007=  "<p>Comentarios de Convenio :".$observacion."</p>";
                 /// $STRING_CADENA_Html_008=  "<p>Correo Gerente: ". $mail_gerente."</p>";
          /////**********************        
          ////***Armamos la informacion Html  a Enviar
               $strin_Fina_HTML =$STRING_CADENA_Html_001.$STRING_CADENA_Html_002 .$STRING_CADENA_Html_003.$STRING_CADENA_Html_004.$STRING_CADENA_Html_005.$STRING_CADENA_Html_006.$STRING_CADENA_Html_007;//.$STRING_CADENA_Html_008;
                    ////****Mandamos  Correo 
                correos("Convenio", "Nuevo Convenio Generado",$mail_gerente,"", $strin_Fina_HTML);    
      ////*******Fin CodigoAgregado  para  Generar   Notificacion 
 ...
 $stringtabla = sprintf("SELECT * FROM detalle_convenio WHERE n_remision=%s ",
...
$tablaservicio3=mysqli_query($conecta1, $stringtabla) or die (mysqli_error($conecta1));
 ...
 /*no entiendo
$queryinsert = sprintf("SELECT * FROM detalle_convenio WHERE n = %s",
GetSQLValueString($codigo, "text"));
...
// echo 'Aplica caja cerrada='.$aplica_cajacerrada;
...
function validarcajacerradaboni(){
var volumen = document.forma1.volumen.value;
var facor_empaque = document.forma1.salpackun.value;
...
<div class="container">
     <form name="forma1" method="POST" action="convenios_representantes.php?folio=<?php echo$_SESSION['foliorep'];?>">
    <div class="row">
        
        <h5>Convenio No. <?php echo $_SESSION['foliorep']; ?></h5>
        <div class="form-group"> 
         
            <select required class ="form-control" id=""  name ="tipo_conV"> 
                <option value ="0" >Convenio</option>
                <option value="1" >Lista Precios</option>
            </select> 
        </div> 
       <div class="col-md-12">
           <br>
           
           <p>Periodo Convenio</p>
           
           <label>Inicio</label>
           <input type="date" name="fechainicio" required value="<?php echo $_SESSION['fechainicio'];?>">
           
           <label>Fin</label>
           <input type="date" name="fechafin" required value="<?php echo $_SESSION['fechafin'];?>">
             <p>Cliente: </p>
        ...
        <div class="col-md-12">
         ...
         <div class="container">
            <div class="row">
             <div class="col-lg-2">
                 <label>Volumen:</label>
                 <input required  name="volumen" id="volumen" type="text" <?php if($aplica_cajacerrada>0){ echo 'onchange="validarcajacerrada()"'; }  ?> class="form-control" >

             </div>

             <div class="col-lg-2">
                 <label>Precio:</label>  
                 <input required  name="preciosub" id="preciosub" type="text" class="form-control"  onchange="calculate();" >
...
<input type="hidden" name="totalservdist" value="<?php echo $totalsd ?>" class="form-control"  readonly >
  ...
  <textarea name="observacion" class="form-control" rows="5" id="comment"></textarea>
    ...
    <div class=" col-lg-12" >
        <p>
            <button name ="guardar" type="submit" class="btn btn-lg btn-primary pull-right" >Guardar Convenio</button> <!--onClick="return confirm('¿Guardar Convenio?');"-->
