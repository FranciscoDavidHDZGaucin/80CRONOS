<?php
////proyecli_addproyeccionesagente_net.php


/*
********	 INFORMACION ARCHIVO ***************** 
	Nombre  Archivo : proycli_addproyeccionage.php
...
{
  $estprocedute =  'ERROR GENERERAR  TABLA '. mssql_get_last_message() ; 
...
////***Nombre del  Procidimiento  a Utilizar 
			$sTOREPRO =   mssql_init("spTABLA_PRESUPUESTOS_PROYECCIONES");
...
$querycliente=sprintf("SELECT * FROM [pedidos].[vwCLIENTES_CRONOS_SAP] WHERE SlpCode=%s OR U_agente2=%s order by  CardName   asc ",
...
/*
$(document).on("key","#CFNNEW",function(){
...
<!--Contenendor  Filtro-->  
 <div  class="col-lg-12 col-sm-12"> 	
          <div  id="REVICIONELEM" hidden > <button   type="button" class="btn btn-info"    id="BTNPRUEBA" >ERROR <span class="glyphicon glyphicon-floppy-disk"></span></button></div>
 </div>
<!--Fin contenedor-->
<!--Contenedor Tabla-->  
 <div  class="col-lg-12 col-md-12 col-sm-12"> 	
 	 ...
 	 <table  class="table   table-bordered " id="dataTables-prouyecc"  >
 	 		<thead>
 	 			<!----> 
 	 			<tr>
                                    <td><button   type="button" class="btn btn-info"    id="saveelem" >Guardar Proyeccion <span class="glyphicon glyphicon-floppy-disk"></span></button>
			...
			<tbody>
				...
				<?php
					...
					/* echo 	 	$string_get_info =sprintf("select * from  pedidos.vwpROYECC_FORMATO_AGREGADO_PROYECCION where num_agente = %s order by cliente",
             				...
             				$string_get_info =sprintf("select  *  from #TEMPORAL_PROYECCIONES where num_agente = %s   order by cliente,familia",
             							GetSQLValueString($_SESSION["usuario_agente"], "int"));
...
if($DETER_TYPE_REGISTRO == TRUE )
{
	...
	/*------------------------COLUMNAS  CONFIRMACION --------------------------------------------------------------------------------------------------------------------------*/		
		...
		if( $rowpresuget['cve_prod'] ==$PROYECMAIN_AGREGADOS['cve_prodCNF'] && $rowpresuget['cve_cliente'] ==$PROYECMAIN_AGREGADOS['cve_clienteCNF'] ){
