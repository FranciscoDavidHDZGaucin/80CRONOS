...
/*
********	 INFORMACION ARCHIVO ***************** 
	Nombre  Archivo : vista-credito-dircom.php 
 	Fecha  Creacion : 23/09/2016
	Descripcion  : 
	Copia  archivo vista-credito-dircom.php  parte  del  Proyecto  Pedidos
	Modificado  Fecha  : 
*/
...
//Funcion para validar si el Pedido se bloquea por credito
 function razonescxc($cardcode,$monto,$dxcx,$tipoventa){
      ...
      $query=("SELECT CardCode, CardName, DocNum, DocDate, DocTotal, PaidToDate, DocTotalFC, PaidSumFC, DocCur, SlpCode, DocDueDate, DocStatus, DATEDIFF(day,DocDueDate,getdate()) as dv  FROM OINV WHERE CardCode='$cardcode' and DocStatus='O' and DATEDIFF(day,DocDueDate,getdate())>120");
   ...
   if ($porce_excede>20 or $monto>400000 or $plazo==1 or $fv30==1 ){   //se valida el monto del pedido
...
<div class="espacio-datos">
	  ...
	  <?php
	  	//mysqli_select_db($database_conecta1,$conecta1);
                ...
                $consulta=sprintf("SELECT * FROM encabezado_x_zona WHERE  vbo_jefecyc=2 and isnull(vbo_dircom) and estatus='C'");
		...
		$Total = mysqli_num_rows ($sql_consulta);
            ?>			
		 <form id="form2" name="form2" method="post" action="vista-credito-dircom.php">
		...
		<?php if($Total<>0) {  ?>
				...
				<?php		while ($reg = mysqli_fetch_array($sql_consulta)) {   ?>
                                                   ...
                                                   if ($autoriza==0 and $n_datos==0){ ?> 
                                                          <td><a href="pop-credito-dircom.php?respuesta=1&id=<?php echo $reg['id']; ?>&remision=<?php echo $reg['n_remision']; ?>&agente=<?php echo $reg['n_agente']; ?>&cliente=<?php echo $reg['cve_cte']; ?>" target="_blank" onClick="window.open(this.href, this.target, 'width=430,height=400,scrollbars=yes'); return false;"><img src="images/thumbup.png" title='Autorizar'> </a> </td>
                                                    
                                                    <?php }?>
                                                   <td><a href="pop-credito-dircom.php?respuesta=2&id=<?php echo $reg['id']; ?>&remision=<?php echo $reg['n_remision']; ?>&agente=<?php echo $reg['n_agente']; ?>&cliente=<?php echo $reg['cve_cte']; ?>" target="_blank" onClick="window.open(this.href, this.target, 'width=430,height=400,scrollbars=yes'); return false;"><img src="images/consulting.png" title='Solicitar Autorización'> </a></td>
