<?php
////****coti_addCotiza_agente.php  
/*
********	 INFORMACION ARCHIVO ***************** 
	Nombre  Archivo : coti_addCotiza_agente.php 
...
{    
 ...
 if(!$qery_folio){ $foliogenerado = 0000000;  }else {     $foliogenerado=$_SESSION["usuario_agente"].$anioactual.$aleatorio;    }
}
////*Validacion para Una Nueva  Cotizacion 
if($TyVisor==1||$TyVisor==2)
{    
$foliogenerado = filter_input(INPUT_POST, 'FOLIO');
    ////*****Solo Utilizamos El FOLIO
    if($TyVisor==2){
        
        ///**oBTENEMOS LA CVE _AGENTE 
        $string_get_CVE = "SELECT cve_agente FROM coti_encabeca_cotizacion  where  folio=".$foliogenerado ;
        $qery_CVE = mysqli_query($conecta1, $string_get_CVE);
        $fetch_Cve = mysqli_fetch_array($qery_CVE);                
        ////****
        $string_get_info = "SELECT com_agent,cve_agente FROM coti_encabeca_cotizacion  where  folio=".$foliogenerado ;
        $qery_info = mysqli_query($conecta1, $string_get_info);
            
        $idagente=$fetch_Cve['cve_agente'];
        $_SESSION["usuario_agente"] =$idagente;
 
    }else{ 
    ///**Obtenemos el N# del  Agente 
    $idagente = $_SESSION["usuario_agente"];
    
    $string_get_info = "SELECT com_agent FROM coti_encabeca_cotizacion  where  folio=".$foliogenerado." and   cve_agente =".$idagente  ;
            $qery_info = mysqli_query($conecta1, $string_get_info);
    }       
...
<script type="text/javascript">
		...
		var   masteFolio  = <?php echo $foliogenerado ; //folio_Cotizacion($_SESSION["usuario_agente"],$conexion); ?>; 
                ///****Numero Agente 
                var   numAgen  = <?php echo $_SESSION["usuario_agente"]; ?>;
                ///***Tipo De Despliegue 
 ...
 ///****BTN  Eliminar elemento  
	...
	$("#boniPreci").change(function(){
        ...
        $("#btnSaALL").click(function(){
            $.ajax({
                                    type:'POST',
                                    url: 'coti_scrip_cotiza/coti_addCabCotiz.php',
                                    data: {"folio":masteFolio,"typDes":typDespl, "nAge" : numAgen, "txtComent":$("#txtcomen").val(),"ZONA":$("#zona").val()}, 
                                    success: function (datos) { 
                                                if(datos.EstQery == true )
                                                {
                                                    ///****Se realiso  la  Insercion 
                                                    window.location.href ="http://192.168.101.5/sistemas/cronos/coti_index_agente_option.php"
                                                }else
                                                {
